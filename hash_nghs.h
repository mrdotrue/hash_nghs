...
template <class HASH> class hashtable {
private:
  ...
  static void clear(eType *A, size_t n, eType v) {
    ...
    parallel_for(0, n, f, parlay::granularity(n));
  }
  ...
  index hashToRange(index h) {
    return static_cast<index>(static_cast<size_t>(h) % m);
  }
...
public:
  ...
  hashtable(size_t size, HASH hashF, double load = 1.5)
      : m(100 + static_cast<size_t>(load * static_cast<double>(size))),
        empty(hashF.empty()), hashStruct(hashF),
        TA(parlay::sequence<eType>(m, empty)) {}
  ...
  size_t count() {
    ...
    return parlay::internal::reduce(parlay::delayed_seq<size_t>(m, is_full),
                                    parlay::addm<size_t>());
  }
  ...
  parlay::sequence<index> get_index() {
    ...
    auto x = parlay::sequence<index>::from_function(m, is_full);
    parlay::internal::scan_inplace(make_slice(x), parlay::addm<index>());
    return x;
  }
...
template <class T> struct hash_numeric {
  using eType = T;
  using kType = T;
